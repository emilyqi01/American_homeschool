#' Calculate Growth rate for Population Data
#'
#' This function calculates the growth rate of home school enrollment for each state
...
#'
#' @return A data frame with the original data plus the calculated growth rate and absolute growth rate.
#'
#' @export
#'
#' @examples
...
#' )
#' calculate_growth_proportion(df, 'state', 'year', 'population')
calculate_growth_proportion <- function(data_frame, state_col, year_col, population_col) {
  ...
  data_frame <- data_frame %>%
    ...
    mutate(
      ...
      proportion = growth / lag(!!sym(population_col)) * 100,
      abs_growth = abs(proportion)
    )
  
  # Remove NA values (growth rate cannot be calculated for the first year as there's no previous year data)
